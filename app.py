...
st.set_page_config(
    page_title=_("🏆 FIFA World Cup 2026 Predictor"), page_icon="⚽", layout="wide"
)
...
def compare_squads(team1, team2, player_stats_df):
    ...
    for pos in positions:
        ...
        results[pos] = {
            ...
            "team2_avg": p2_avg,
            "advantage": (
                "team1" if p1_avg > p2_avg else "team2" if p2_avg > p1_avg else "equal"
            ),
        }
    ...
    squad_advantage = (
        ...
        else "team2" if total_team2 > total_team1 else "equal"
    )
...
def calculate_win_probability(team1, team2):
    ...
    squad_results, squad_advantage, squad1_total, squad2_total = compare_squads(
        team1, team2, player_stats
    )
...
def call_ai(prompt, context=""):
    if ai_provider == "ADK Agent":
        ...
        try:
            ...
            result = runner.run(session_id=session_id, input=full_prompt)
        ...
        try:
            response = openai_client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": _("You are a football expert assistant."),
                    },
...
def get_worldcup_matches(show_probabilities=False):
    ...
    for m in matches:
        ...
        if show_probabilities:
            ...
            row[_("Win %")] = (
                f"{prob['team1_win']}% / {prob['draw']}% / {prob['team2_win']}%"
            )
...
st.caption(
    _("Live scores, standings, and AI predictions for the World Cup | AI: {}").format(
        ai_provider
    )
...
auto_refresh = st.sidebar.checkbox(
    _("Auto-refresh live scores (every 30 sec)"), value=False
)
if auto_refresh:
...
if mode == _("World Cup 2026"):
    ...
    with tab1:
        ...
        if matches:
            ...
            if st.button(_("🤖 AI World Cup Analysis")):
                ...
                analysis = call_ai(
                    _("Analyze which match is most exciting and predict winners."), ctx
                )
                st.info(analysis)
        else:
    ...
    with tab2:
        ...
        if not df_stand.empty:
            for grp in sorted(df_stand["Group"].unique()):
                ...
                group_df = df_stand[df_stand["Group"] == grp].rename(
                    columns={
                        "Group": _("Group"),
                        "Team": _("Team"),
                        "Played": _("Played"),
            ...
            if st.button(_("🤖 AI Group Analysis")):
                analysis = call_ai(
                    _("Which teams are likely to advance to knockout stage?"),
                    f"{_('Standings')}:\n{df_stand.to_string()}",
                )
                st.info(analysis)
        else:
    ...
    st.markdown(
        "All data is drawn **locally** from `worldcup.squads.json` and `worldcup.json`."
    )
    ...
    if not df_stand.empty:
        ...
        fig = px.bar(
            df_display, x="Team", y="Points", color="Group", title="Points per Team"
        )
    ...
    df_top = build_player_stats()[["Player", "Goals"]].sort_values(
        "Goals", ascending=False
    )
    if not df_top.empty:
        st.dataframe(df_top, use_container_width=True)
        fig = px.bar(
            df_top.head(10), x="Player", y="Goals", title="Top 10 Goal Scorers"
        )
        st.plotly_chart(fig, use_container_width=True)
    else:
    ...
    st.markdown(
        "Select two teams to get a comprehensive win probability analysis based on historical data, head-to-head, squad comparison, and current form."
    )
    ...
    if not all_teams:
        ...
        with col2:
            team2 = st.selectbox(
                _("Select Team 2"), all_teams, index=min(1, len(all_teams) - 1)
            )
        ...
        if team1 and team2 and team1 != team2:
            ...
            if (
                "predict_result" in st.session_state
                and st.session_state["predict_result"]
            ):
                result = st.session_state["predict_result"]

                st.markdown(
                    f"### 📊 Win Probability: {result['team1']} vs {result['team2']}"
                )
                ...
                with st.expander(_("📈 Detailed Breakdown")):
                    ...
                    st.write(
                        f"**{result['team1']}** win rate in World Cups: {result['historical_win_pct1']}%"
                    )
                    st.write(
                        f"**{result['team2']}** win rate in World Cups: {result['historical_win_pct2']}%"
                    )
                    ...
                    st.write(
                        f"**{result['team1']}** squad score: {result['squad_score1']}"
                    )
                    st.write(
                        f"**{result['team2']}** squad score: {result['squad_score2']}"
                    )
                    ...
                    for pos, data in pos_comp.items():
                        st.write(
                            f"**{pos}**: {result['team1']} avg {data['team1_avg']:.1f} vs {result['team2']} avg {data['team2_avg']:.1f} → {data['advantage']} advantage"
                        )
...
st.caption(
    _(
        "Data: openfootball/worldcup.json (matches & squads). Historical data from openfootball/worldcup. AI powered by Ollama/OpenAI/ADK."
    )
)
