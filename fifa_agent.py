...
def get_standings_text():
    ...
    for (group, team), data in standings.items():
        if data["played"] > 0:
            lines.append(
                f"Group {group}: {team} – P{data['played']} W{data['won']} D{data['drawn']} L{data['lost']} GF{data['gf']} GA{data['ga']} Pts{data['pts']}"
            )
...
def predict_match_outcome(team1: str, team2: str) -> str:
    ...
    if s1 > s2 + 5:
        ...
        return (
            f"The match between {team1} and {team2} is expected to be close or a draw."
        )
